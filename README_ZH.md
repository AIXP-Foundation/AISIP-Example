# SoulBot (Rebirth Edition): 基于 AISIP 协议的自主 AI 重生版
...
**SoulBot** 是 **[AISIP V1.0.0](https://aisip.dev)**（AI Standard Instruction Protocol，AI 标准指令协议）的官方参考实现。它展示了如何向极简的 Python 运行时注入 AISIP「灵魂」（`.aisip.json` 文件），将业务逻辑与代码完全解耦，构建出真正智能的 AI Agent。

[**English**](README.md) | [**AISIP Protocol**](https://aisip.dev)
...
* **专业级 AISIP 定制**：允许用户自定义专业领域的 `.aisip.json` 文件，瞬间将智能体转化为特定行业的顶级专家。
...
* **自演化与自我升级 (Self-Evolutionary & Self-Upgrading)**：依托 **AISIP 协议**，SoulBot 能够自主修改自身的逻辑（AISIP）与执行代码，实现 **自我进化 (Self-Evolution)** 的闭环。
* **自我分形与分形智能 (Self-Fractal & Fractal Intelligence)**：SoulBot 的架构本质上是递归的。AISIP 可以派生并链接其他 AISIP，构建出一个跨节点、跨领域的 **分形智能** 网络，实现自主扩展。
...
* **100% 协议驱动** — 所有行为由 `.aisip.json` 文件定义，Python 中零业务逻辑
* **双界面** — Telegram Bot + Streamlit Web UI，统一入口一键启动
* **多模型支持** — OpenCode ACP（免费模型 + 付费模型）、Claude ACP、Gemini ACP、Cursor CLI
* **自分形架构** — AISIP 调用 AISIP：`main → stock_analysis → stock_data_fetcher`
* **自我升级** — AI 可在运行时创建和修改 `.aisip.json` 文件
...
| **灵魂 (Soul)** | 纯 JSON AISIP 文件，定义人设、技能、工作流和路由。 | `aisip/*.aisip.json` |
| **大脑 (Brain)** | 通过 ACP（Agent Client Protocol）访问的 LLM 引擎，读取 AISIP 文件并驱动行为。 | OpenCode / Claude / Gemini / Cursor CLI |

---

## 设计理念

> "Ghost in the Shell" — 当 Python 只是躯壳，AISIP 才是灵魂。
> **SoulBot 采用"玻璃盒架构 (Glass-box Architecture)"，确保自演化过程绝对透明。**

传统 AI Bot 把逻辑、Prompt 和工具调用硬编码在 Python `if/else` 里。SoulBot 颠覆了这一模式：

1. **躯壳与灵魂分离** — Python 只负责 I/O，AISIP 文件定义一切行为
...
│
├── aisip/                      # [灵魂] AISIP 文件
│   ├── main.aisip.json         # 核心路由器（意图识别、任务分发）
│   ├── stock_analysis.aisip.json
│   ├── stock_data_fetcher.aisip.json
│   ├── weather.aisip.json
│   ├── main.aisip.md           # AISIP 说明文档
...
SoulBot 的行为完全由 `.aisip.json` 文件驱动。主 AISIP 充当路由器：
...
用户消息 → 分析意图 → 路由
  ...
  ├── 提问    → 直接回答
  ├── 任务    → 执行任务
  ├── 股票    → 验证代码 → 运行 stock_analysis.aisip.json
  └── 天气    → 验证地点 → 运行 weather.aisip.json
```

你可以为任何领域创建新的 AISIP 文件 — 让 AI 生成，或手动编写。无需修改任何 Python 代码。
...
| **运行时** | | |
| `SYSTEM_PROMPT` | AISIP 运行时系统指令 | *(内置)* |
| `WORKSPACE_DIR` | AI 工具的工作目录 | `aisip` |
...
为任何场景创建 `.aisip.json` 文件：

* `code_review.aisip.json` — 代码审查和最佳实践
* `daily_report.aisip.json` — 多源信息聚合日报
* `fitness.aisip.json` — 健身追踪和建议
