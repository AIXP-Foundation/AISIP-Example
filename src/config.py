...
# Resolve WORKSPACE_DIR relative to project root (not CWD), so "aisip" always maps to <project>/aisip
...
# ── AISIP Loading & System Instruction (shared by Telegram handler and Web UI) ──
...
def clear_instruction_cache():
    """清除系统指令缓存（配置更新后调用）"""
    global _system_instruction_cache, _aisip_cache
    _system_instruction_cache = None
    _aisip_cache = {}


def load_aisip(name="main"):
    """加载 AISIP 文件（带缓存，路径基于 PROJECT_ROOT）"""
    if name in _aisip_cache:
        return _aisip_cache[name]

    try:
        path = os.path.join(PROJECT_ROOT, "aisip", f"{name}.aisip.json")
        with open(path, "r", encoding="utf-8-sig") as f:
            aisip = json.load(f)
            _aisip_cache[name] = aisip
            return aisip
    except Exception as e:
        logger.error(f"Failed to load aisip {name}: {e}")
        return None


# Backwards compatibility alias
load_blueprint = load_aisip
...
def get_system_instruction() -> str:
    ...
    aisip = load_aisip("main")
    aisip_str = json.dumps(aisip, ensure_ascii=False, indent=2) if aisip else "{}"
    workspace_line = f"\n\n[WORKSPACE]\nYour AISIP directory is: {WORKSPACE_DIR}"

    if base_prompt and aisip:
        _system_instruction_cache = (
            f"{base_prompt}\n\n"
            f"---\n"
            f"[LOADED AISIP: main.aisip.json]\n"
            f"```json\n{aisip_str}\n```"
            f"{workspace_line}"
        )
    elif base_prompt:
