# SoulBot (Rebirth Edition) — The Official AISIP Protocol Reference

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/downloads/)
[![AISIP V1.0.0](https://img.shields.io/badge/protocol-AISIP%20V1.0.0-green)](https://aisip.dev)
...
**SoulBot** is the official reference implementation of the **[AISIP V1.0.0](https://aisip.dev)** (AI Standard Instruction Protocol). It demonstrates how to build an intelligent AI Agent by injecting an AISIP "Soul" (`.aisip.json` files) into a minimal Python runtime — completely decoupling logic from code.

[**中文说明**](README_ZH.md) | [**AISIP Protocol**](https://aisip.dev)
...
* **Domain-Specific Souls**: Create custom professional `.aisip.json` files to instantly transform your agent into a field expert.
...
* **Self-Evolutionary & Self-Upgrading**: Leveraging the **AISIP Protocol**, SoulBot can autonomously refine its own logic (AISIPs) and execution code, achieving a closed loop of **Self-Evolution**.
* **Self-Fractal & Fractal Intelligence**: The architecture is inherently recursive. AISIPs can spawn and interconnect with other AISIPs, creating a **Fractal Intelligence** network that scales autonomously across nodes.
...
* **100% Protocol-Driven** — All behavior defined by `.aisip.json` files, zero business logic in Python
...
* **Self-Fractal Architecture** — AISIPs calling AISIPs: `main → stock_analysis → stock_data_fetcher`
* **Self-Upgrading** — AI can create and modify `.aisip.json` files at runtime
...
| **Soul** | Pure JSON aisip files defining persona, skills, workflow, and routing. | `aisip/*.aisip.json` |
| **Brain** | LLM engine accessed via ACP (Agent Client Protocol). Reads aisip files and drives behavior. | OpenCode / Claude / Gemini / Cursor CLI |

---

## Design Philosophy

> "Ghost in the Shell" — When Python is just the shell, AISIP is the soul.
> **SoulBot adopts a "Glass-box Architecture," ensuring absolute transparency in the evolution process.**

Traditional AI bots hardcode logic, prompts, and tool calls in Python `if/else` statements. SoulBot disrupts this pattern:

1. **Separation of Shell and Soul** — Python only handles I/O; aisip files define all behavior.
...
```bash
git clone https://github.com/AIXP-Foundation/AISIP-Exemple.git
cd AISIP-Exemple
...
│
├── aisip/                      # [Soul] AISIP files
│   ├── main.aisip.json         # Core router (intent detection, task delegation)
│   ├── stock_analysis.aisip.json
│   ├── stock_data_fetcher.aisip.json
│   ├── weather.aisip.json
│   ├── main.aisip.md           # AISIP documentation
│   ├── stock_analysis.aisip.md
│   └── weather.aisip.md
│
...
SoulBot's behavior is entirely driven by `.aisip.json` files. The main aisip acts as a router:
...
User Message → Analyze Intent → Route
  ...
  ├── Task      → Execute task
  ├── Stock     → Validate ticker → Run stock_analysis.aisip.json
  └── Weather   → Validate location → Run weather.aisip.json
```

You can create new aisip files for any domain — tell the AI to generate them, or write them manually. No Python code changes needed.
...
| **Runtime** | | |
| `SYSTEM_PROMPT` | AISIP runtime system instruction | *(built-in)* |
| `WORKSPACE_DIR` | Working directory for AI tools | `aisip` |
...
Create new `.aisip.json` files for any use case:

* `code_review.aisip.json` — Code analysis and best practices
* `daily_report.aisip.json` — Aggregate info from multiple sources
* `fitness.aisip.json` — Workout tracking and suggestions

**Protocol > Code.** Change behavior by editing JSON, not Python.

---

*Powered by [AISIP Protocol](https://aisip.dev)*

Copyright (c) 2026 AIXP Foundation AIXP.dev | AISIP.dev
